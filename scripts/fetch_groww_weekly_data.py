#!/usr/bin/env python3
"""
Groww Historical Weekly Data Fetcher - Production Ready
========================================================
Fetches weekly OHLCV data from Groww API and caches locally.
...
CACHE_DIR = Path("data/cache/groww/weekly")
CACHE_DIR_DAILY = Path("data/cache/groww/daily")
...
WEEKLY_DATA_CUTOFF = datetime(2015, 11, 9)  # Historical data available from this date
...
def fetch_weekly_data(
    ...
    """Fetch weekly OHLCV data from Groww API.
    ...
    Returns:
        DataFrame with weekly OHLC data or None if fetch fails
    """
    try:
...
def save_weekly_data(df: pd.DataFrame, exchange_token: str, symbol: str) -> bool:
    ...
    try:
        ...
        output_file = CACHE_DIR / f"groww_{exchange_token}_{symbol}_1w.csv"
...
def fetch_basket_weekly_data(basket_name: str, access_token: str) -> Dict[str, int]:
    """Fetch weekly data for all symbols in a basket.
    ...
    for symbol in sorted(symbols):
        ...
        if df is not None:
            # Save to cache
            if save_weekly_data(df, info['exchange_token'], symbol):
                success_count += 1
            else:
                error_count += 1
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch weekly OHLCV data from Groww API",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Fetch specific basket
  python3 fetch_groww_weekly_data.py --basket large
  
  # Fetch specific symbols
  python3 fetch_groww_weekly_data.py --symbols RELIANCE,INFY,TCS
  
  # Fetch all baskets
  python3 fetch_groww_weekly_data.py --all-baskets
        """,
    )
    ...
    print("\n" + "="*70)
    print("GROWW WEEKLY DATA FETCHER")
    print("="*70)
    ...
    if args.all_baskets:
        for basket_name in BASKETS.keys():
            results[basket_name] = fetch_basket_weekly_data(basket_name, access_token)
    elif args.basket:
        results[args.basket] = fetch_basket_weekly_data(args.basket, access_token)
    elif args.symbols:
        ...
        for symbol in symbols:
            info = get_symbol_info(symbol)
            if info:
                df = fetch_weekly_data(symbol, info['exchange_token'], access_token)
                if df is not None and save_weekly_data(df, info['exchange_token'], symbol):
                    success_count += 1
                else:
                    error_count += 1
            else:
    ...
    print("\n📁 Weekly data stored in data/cache/groww/weekly/ as groww_{EXCHANGE_TOKEN}_{SYMBOL}_1w.csv")
    print("="*70 + "\n")
