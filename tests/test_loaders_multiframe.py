...
from data.loaders import load_ohlc_dhan_multiframe, load_many_dhan_multiframe
...
def test_single_loader():
    ...
    for symbol, timeframe in test_cases:
        try:
            ...
            assert (df["high"] >= df["open"]).all() or (df["high"] >= df["close"]).all(), "Invalid high"
    ...
    for timeframe in ["75m", "125m", "1d"]:
        ...
        try:
            ...
            for symbol, df in sorted(data_dict.items()):
                print(f"  - {symbol:12}: {len(df):4d} candles | Price: ₹{df['close'].mean():.2f} avg")
            
            # Validate
            assert len(data_dict) > 0, "No data loaded"
            assert all(isinstance(df, pd.DataFrame) for df in data_dict.values()), "Not all DataFrames"
...
def test_candle_counts():
    ...
    print(f"  📊 75m  data: {len(df_75m):4d} candles | {trading_days_75m} trading days | {len(df_75m)/trading_days_75m:.1f} candles/day ✓")
    print(f"  📊 125m data: {len(df_125m):4d} candles | {trading_days_125m} trading days | {len(df_125m)/trading_days_125m:.1f} candles/day ✓")
    print(f"  📊 1d   data: {len(df_1d):4d} candles | {trading_days_1d} trading days | 1.0 candles/day ✓")
